...
type CartRedisRepository interface {
	...
	SetItem(ctx context.Context, userID string, item model.CartItem) error
	...
	GetItem(ctx context.Context, userID string, skuID string) (*model.CartItem, error)
	...
	SetCartFromItems(ctx context.Context, userID string, items []model.CartItem) error
}
