...
type AttributeOption struct {
	...
	Label map[string]string `bson:"label" json:"label"` // {"en": "Red", "vi": "Do"}
	Color string            `bson:"color,omitempty" json:"color,omitempty"` // Hex color for COLOR type
...
type AttributeDefinition struct {
	...
	Name         map[string]string    `bson:"name" json:"name"` // {"en": "Color", "vi": "Mau sac"}
	Type         string               `bson:"type" json:"type"` // TEXT, NUMBER, SELECT, MULTI_SELECT, BOOLEAN, COLOR, SIZE
	Unit         string               `bson:"unit,omitempty" json:"unit,omitempty"` // kg, cm, GB, etc.
	...
	IsVariant    bool                 `bson:"isVariant" json:"isVariant"` // Used for product variants
...
type Category struct {
	...
	Path     string              `bson:"path" json:"path"` // /electronics/phones/smartphones
	Level    int                 `bson:"level" json:"level"` // 0, 1, 2, 3
	Position int                 `bson:"position" json:"position"` // Sort order

	// Basic Info (multilingual)
	Name        map[string]string `bson:"name" json:"name"` // {"en": "Phones", "vi": "Dien thoai"}
	...
	CommissionRate float64 `bson:"commissionRate" json:"commissionRate"` // Platform fee %

	// Status
	Status     string `bson:"status" json:"status"` // ACTIVE, INACTIVE, HIDDEN
