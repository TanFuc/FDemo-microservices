...
func LoadSharedPackagesConfig(mainCfg *Config) *SharedPackagesConfig {
	...
	return &SharedPackagesConfig{
		Cache: CachePackageConfig{
			...
			ServiceName:   getEnvString("CACHE_SERVICE_NAME", "catalog-service"),
		...
		Authorization: AuthorizationPackageConfig{
			...
			ServiceName:   getEnvString("AUTHZ_SERVICE_NAME", "catalog-service"),
		...
		CustomFields: CustomFieldsPackageConfig{
			...
			ServiceName:   getEnvString("CUSTOMFIELDS_SERVICE_NAME", "catalog-service"),
...
// Helper functions already defined in config.go, using internal versions here
