...
// It also accepts the main config to use existing Redis settings as defaults
func LoadSharedPackagesConfig(mainCfg *Config) *SharedPackagesConfig {
	// Build redis address from main config if available
	redisAddr := "localhost:6379"
	redisPassword := ""
	redisDB := 0
	if mainCfg != nil {
		redisAddr = fmt.Sprintf("%s:%d", mainCfg.Redis.Host, mainCfg.Redis.Port)
	...
	return &SharedPackagesConfig{
		Cache: CachePackageConfig{
			...
			ServiceName:   getEnvString("CACHE_SERVICE_NAME", "api-gateway"),
		...
		Authorization: AuthorizationPackageConfig{
			...
			ServiceName:   getEnvString("AUTHZ_SERVICE_NAME", "api-gateway"),
