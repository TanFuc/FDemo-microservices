...
// @Produce json
// @Param X-User-ID header string true "User ID"
// @Param request body service.RegisterShopInput true "Shop details"
// @Success 200 {object} response.Response
// @Failure 400 {object} response.ErrorResponse
...
func (h *ProfileHandler) RegisterShop(c *fiber.Ctx) error {
	...
	var input service.RegisterShopInput
	...
	profile, err := h.profileService.RegisterShop(c.Context(), userID, &input)
	if err != nil {
		return response.Error(c, err)
	}
...
// @Produce json
// @Param X-User-ID header string true "User ID"
// @Param request body service.UpdateShopInput true "Shop updates"
// @Success 200 {object} response.Response
// @Failure 400 {object} response.ErrorResponse
...
func (h *ProfileHandler) UpdateShop(c *fiber.Ctx) error {
	...
	var input service.UpdateShopInput
	if err := c.BodyParser(&input); err != nil {
		return response.BadRequest(c, "Invalid request body")
	}

	profile, err := h.profileService.UpdateShop(c.Context(), userID, &input)
	if err != nil {
		return response.Error(c, err)
	}
...
// @Produce json
// @Param X-User-ID header string true "User ID"
// @Param request body service.CreateAddressInput true "Address details"
...
func (h *ProfileHandler) CreateAddress(c *fiber.Ctx) error {
	...
	address, err := h.profileService.AddAddress(c.Context(), userID, &input)
	if err != nil {
		return response.Error(c, err)
	}
