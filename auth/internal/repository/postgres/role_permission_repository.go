...
func (r *rolePermissionRepository) FindByRoleIDWithPermissions(ctx context.Context, roleID int64) ([]model.RolePermission, error) {
	var rps []model.RolePermission
	err := r.db.WithContext(ctx).
		Preload("Permission").
		Where("role_id = ?", roleID).
...
func (r *rolePermissionRepository) GetUserPermissions(ctx context.Context, userID uuid.UUID) ([]string, error) {
	...
	for _, ur := range userRoles {
		if ur.Role.Name == model.RoleSuperAdmin {
			return []string{"*"}, nil
		}
	}
