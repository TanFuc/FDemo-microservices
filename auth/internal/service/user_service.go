...
func (s *UserService) CreateUser(ctx context.Context, input *CreateUserInput) (*model.User, error) {
	...
	if err != nil {
		...
		defaultRole, err = s.roleRepo.FindByName(ctx, model.RoleUser)
		if err != nil {
...
func (s *UserService) GetUserWithRoles(ctx context.Context, userID uuid.UUID) (*model.User, error) {
...
func (s *UserService) RecordLoginHistory(ctx context.Context, userID uuid.UUID, status model.LoginStatus, authMethod model.AuthMethod, ipAddress string, userAgent *string) error {
	history := model.NewLoginHistory(userID, status, authMethod, ipAddress, userAgent)
