...
func (s *SeedingService) SeedRoles(ctx context.Context) error {
	...
	logger.Info().Int("count", len(model.DefaultRoles)).Msg("Roles seeded successfully")
	return nil
}
...
func (s *SeedingService) SeedPermissions(ctx context.Context) error {
	...
	for _, permission := range model.DefaultPermissions {
	...
	logger.Info().Int("count", len(model.DefaultPermissions)).Msg("Permissions seeded successfully")
	return nil
}
...
func (s *SeedingService) SeedRolePermissions(ctx context.Context) error {
	...
	for roleName, permissionSlugs := range model.RolePermissionMapping {
...
func (s *SeedingService) ResyncRolePermissions(ctx context.Context, roleName string) error {
	...
	// Re-add permissions
	permissionSlugs, ok := model.RolePermissionMapping[roleName]
	if !ok {
		return nil
	}
