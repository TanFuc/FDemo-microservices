...
func (s *AuthService) Login(ctx context.Context, input *LoginInput, deviceInfo *DeviceInfo) (*LoginResponse, error) {
	...
	if err != nil {
		...
		if user != nil {
			...
			s.userService.RecordLoginHistory(ctx, user.ID, entity.LoginStatusFailed, entity.AuthMethodPassword, deviceInfo.IPAddress, &ua)
		}
		return nil, err
	}
	...
	ua := deviceInfo.UserAgent
	s.userService.RecordLoginHistory(ctx, user.ID, entity.LoginStatusSuccess, entity.AuthMethodPassword, deviceInfo.IPAddress, &ua)
...
func (s *AuthService) ValidateUser(ctx context.Context, userID uuid.UUID) (*entity.User, error) {
